 ...
 *
 * The MSP430G2553 connects directly to the Panel (using 6 I/O pins!)
 * The 6 I/O pins are the low 6 pins of the "LCDPORT" ( P2.5...P2.0)
 *
 * CONNECTIONS:
 *
 * MSP430			  			Panel
 ...
 * LCDPORT.5                    E (enables read/write)
 *
 * LCDPORT Bits:
 * Bit:         7   6   5   4   3   2   1   0
 * Controller:  x   x   E   RS  D7  D6  D5  D4
 *
 * NOTE: the LCD RW pin is grounded, putting the panel in read-only mode
 *
 * TIMING:
...
#define RS 0x10
#define ENABLE_READ 0x20
// Shifted bit position for RS to LCD_put
#define RSINT (16*RS)

void init_LCD_ports(){
	// setup ports for LCD
	LCDPORT(OUT) &= ~(DATABITS+RS+ENABLE_READ);
	LCDPORT(DIR)|=DATABITS+RS+ENABLE_READ;
}

/* LCD PANEL INTERACTIONS
 * LCD_put_4(data) writes 4 bits to the LCD, using the current
 *           RS in the output registers
...
void start_LCD_put_4(unsigned char data){  // write 4 data bits keeping current RS flag
	...
	LCDPORT(OUT)|=ENABLE_READ; // latch data into controller
...
void init_put_word(unsigned int value){ // start the send of the current word (code shared by LCD_put and LCD_send_buffer
	...
	LCD_step=1;
	high = value/16;  // 0 0 0 RS DB7 DB6 DB5 DB4
	LCDPORT(OUT) = ( LCDPORT(OUT) &0xE0) | (high & 0x1F) ; // send out 5 high bits to lines
	LCDPORT(OUT)|=ENABLE_READ; // latch data into controller
